...
app.controller('PopupCtrl', ['$scope', function($scope) {
	...
	chrome.storage.local.get({'active': false, 'urls': [], 'domains': '*', 'exposedHeaders': ''}, function(result) {
		$scope.active = result.active;
		$scope.urls = result.urls;
		$scope.domains = result.domains; 
		...
		$scope.$watch('domains', function(newValue, oldValue) {
			chrome.storage.local.set({'domains': $scope.domains});
			chrome.extension.getBackgroundPage().reload();
		});
